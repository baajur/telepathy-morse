...
void MorseConnection::onAvatarRequestFinished(Telegram::Client::FileOperation *fileOperation, const Peer &peer, QBuffer *device)
{
    const Telegram::FileInfo *fileInfo = fileOperation->fileInfo();
    const QString fileId = fileInfo->getFileId();
    qDebug() << Q_FUNC_INFO << fileId;
    if (m_peerPictureRequests.contains(fileId)) {
        if (!peerIsRoom(peer)) {
            avatarsIface->avatarRetrieved(peer.id, fileId, device->data(), fileInfo->mimeType());
        } else {
...
void MorseConnection::requestAvatars(const Tp::UIntList &contacts, Tp::DBusError *error)
{
    ...
    foreach (quint32 handle, contacts) {
        ...
        Telegram::Client::FileOperation *fileOperation = m_client->filesApi()->downloadFile(&pictureFile, device);
        fileOperation->connectToFinished(this, &MorseConnection::onAvatarRequestFinished,
                              fileOperation, peer, device);
